...
#### 2b. Originality
* This data is  on originality as the participants who submitted data were the company's participants.

#### 2c. Comprehensiveness
* This data is medium on comprehensiveness the data collected match parameters that Bellabeat needs. Having more data on the participants (age, race, health status) would greatly improve any analysis report we produce.
...
#### 2e. Cited
* This data is low on cited. The data was collected through third party making the cited unknown.

## 3. Process

### Setting up my enviornment
Notes: setting up my environment by installing and loading the 'tidyverse', 'skimr', 'plotly', 'dplyr', and 'ggplot2'  package
...
This graph shows users recorded the most steps during the weekday (Tuesday - THursday). This may be due to their occupation (nurse or kitchen worker) or way of transportation.

#### Graph 2: Percentage of Activity Levels
This graph shows the users total activity levels throughout their recording.
...
```
We see that users spend 81% of their day in a sedentary level. 29% of the time is recorded as some form of activity.
